...
describe('gulp-protractor-cucumber-html-report', function () {
  describe('in buffer mode', function () {
    ...
    it('should create separate reports for each file buffer', function (done) {
      var stream = reporter({
        dest: 'output'
      });
    ...
    it('Should handle multiple screenshots correctly', function (done) {
      var stream = reporter({
        dest: 'output'
      });

      var jsonFile = createFile('./data/multiple_screenshots_report.json');

      console.log(jsonFile);

      var expectedReportBuffer = fs.readFileSync(path.join(__dirname, './data/multiple_screenshots_expected_report.html'));

      stream.on('data', function () {
        var resultBuffer = fs.readFileSync(outputFolder + '/multiple_screenshots_report.html');
        assert.equal(resultBuffer.toString(), expectedReportBuffer.toString());
      });
